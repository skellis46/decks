...
- Richest material, hardest to share responsibly
- Controlled tier: redacted dataset + coding framework + coding matrix only
- The NVivo project itself will never be released

[Visual: tiers diagram — Tier 1 open metadata / Tier 2 open aggregates / Tier 3 controlled case-level access. Good candidate for a simple 3-row table or pyramid in reveal.js.]

Note:
The open-text fields are analytically the richest material in the dataset and the hardest to make available responsibly. Under the draft access guidelines, the controlled tier (case-level, signed agreement) holds only: the redacted case-level dataset (XLSX), the redacted coding framework, and a derived coding matrix (case ID × theme label, containing no verbatim text). Single-case themes in clusters 03/05/06 are reviewed and suppressed or merged where they could re-identify a respondent.
